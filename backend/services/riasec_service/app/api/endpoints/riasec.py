...
async def unified_chat(
    ...
    if not session:
        ...
        # Sinh câu hỏi đầu tiên đồng bộ qua Gemini
    ...
    # Trích xuất tín hiệu RIASEC qua Gemini
    ...
    if should_stop(conf, current_scores, session.question_count):
        ...
        # Chạy tác vụ nền để Gemini phân tích báo cáo chi tiết cuối cùng
        background_tasks.add_task(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code == 200:
                ...
                logger.error(f"[GEMINI FLOW] Failed to fetch internal user data for result: {resp.text}")
    except Exception as e:
        logger.error(f"[GEMINI FLOW] Error calling profile_service for user result: {e}")
